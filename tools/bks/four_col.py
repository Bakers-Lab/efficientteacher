...
def read_files(model_name: str,
    ...
    # gt_file = open(os.path.join(input_path, "TPMBDBDL.csv"), 'r')
    # pred_file = open(os.path.join("result","TPMBDBDL.csv"), 'r')

    pred_file= open(os.path.join(input_path, "TPMBDBDL.csv"), 'r')
    # gt_file = open(os.path.join("input","gt.csv"), 'r')
    gt_file=open("/home/ykn@corp.sse.tongji.edu.cn/mmyolo/work_dirs/yolov8_s_syncbn_fast_8xb16-500e_ykn/export-01/TGT.csv","r")

    img_path=os.path.join("data","rb_ins")
    paths_file = os.path.join("data","rb_ins","test.json")
    with open(paths_file, 'r') as json_file:
        paths_file = json.load(json_file)
    
    paths=paths_file['images']
    paths_lines=[None]*len(paths)
    for idx, path in enumerate(paths):
        if idx!=path['id']:
            raise TypeError("error")
        paths_lines[idx]=os.path.join(img_path,"test", path['file_name'])
    ...
    gt_lines, pred_lines = gt_file.readlines(), pred_file.readlines()
    gt_file.close(), pred_file.close()
...
def calc_indicators(model_name: str,
    ...
    gt, pred, paths = read_files(model_name, label_names, input_path, merge, is_dup)
...
def read_labels(input_path):
    file = open(os.path.join(input_path,"DATA.yaml"))
...
def get_args():
    ...
    # parser.add_argument('-t', type=str, help="gt path",required=True)
...
if __name__ == '__main__':
    arg=get_args()
    input_path = arg.d
    out_path = os.path.join(input_path,"4cols")
