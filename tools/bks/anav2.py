            ...
            for k in range(batch_size):
                ...
                if plot_over_kill is not None:
                    ...
                    for o in oks:
                        ...
                        if o[6] >= conf_thres:
                            if merge:
                                box_text = label_names[c] + ': ' + str(o[7]) if draw_label else str(int(o[7]))
                            else:
...
def analyse_visual_result():
    base_path = '/home/tanrui@corp.sse.tongji.edu.cn/avi-defect-detection/experiments/tanrui/yolov5/runs/val/exp21'
    pref, suff = 'val_batch', ['_labels.jpg', '_missing.jpg', '_overkill.jpg', '_pred.jpg']
    for i in range(7):
        label = cv2.imread(os.path.join(base_path, pref + str(i) + suff[0]))
        pred = cv2.imread(os.path.join(base_path, pref + str(i) + suff[3]))
        missing = cv2.imread(os.path.join(base_path, pref + str(i) + suff[1]))
        overkill = cv2.imread(os.path.join(base_path, pref + str(i) + suff[2]))
        h, w = label.shape[0], label.shape[1]
        img_k = None
        for j in range(8):
            start, end = int(j * w / 8), int((j + 1) * w / 8)
            l = label[:, start:end, :]
            p = pred[:, start:end, :]
            m = missing[:, start:end, :]
            o = overkill[:, start:end, :]
            if img_k is None:
                img_k = np.hstack((l, p, m, o))
            else:
                img_k = np.vstack((img_k, np.hstack((l, p, m, o))))
        cv2.imwrite(os.path.join(base_path, pref + str(i) + '.jpg'), img_k)


# 合并几张图的同一列，例如把conf0.1和conf0.0的pred列合在一起
# 默认取第一张图的第一列作为GT贴在合并结果最左侧
def merge_results(paths: list, out_path, merge_column=1, column_width=1000):
    if len(paths) <= 0:
        return
    images = []
    for i in paths:
        images.append(cv2.imread(i))
    gt = images[0][:, :column_width, :]
    for i in images:
        gt = np.hstack((gt, i[:, merge_column * column_width:(merge_column + 1) * column_width, :]))
...
label_filter = {
    ...
    'rb_YiWu': [10, 10],
    'rb_ShunHuai': [10, 10],
    'rb_HuaShangLight': [100, 10],
    'rb_HuaShangMiddle': [10, 1],
    'rb_HuaShangHeavy': [0, 0],
    'rb_YaShangLight': [100, 10],
    'rb_YaShangHeavy': [10, 10],
    'rb_LuTongLight': [100, 10],
    'rb_LuTongHeavy': [100, 1],
    'rb_ZangWuLight': [1, 1],
    'rb_ZangWuMiddle': [10, 10],
    'rb_ZangWuHeavy': [0, 0],
    'other': [0, 0]
}


def write_filter_merge(preds_merge):
    preds_merge[:, :4] = xyxy2xywh(preds_merge[:, :4])
    filtered = []
    with open('result_txt/filter_merge.txt', 'w') as f:
        count = 0
        for p in preds_merge:
            box_id = get_box_id(p)
            cls = int(p[5])
            area = p[2] * p[3] * 100
            conf = float(p[-2]) * 100
            thres = label_filter[label_names[cls]]
            if area <= thres[0] and conf <= thres[1]:
                f.write(box_id + '\n')
                filtered.append(box_id)
                count += 1
        # print(count)
        f.close()
    return filtered
...
def filter_merge(preds, filtered, out_path='result_txt/pred_yolo_filtered.txt'):
    preds[:, :4] = xyxy2xywh(preds[:, :4])
    count = 0
    with open(out_path, 'w') as f:
        for p in preds:
            box_id = get_box_id(p)
            if box_id not in filtered:
                count += 1
                coord = ''
                for c in p[:4]:
                    coord += format_float(c) + ','
                f.write(coord + format_float(p[-2]) + ',' + str(int(p[4])) + ',' + str(int(p[5])) + ',' +
                        format_float(p[2] * p[3] * 100) + ',' + str(int(p[-1])) + '\n')
            else:
                print(box_id)
        f.close()
        # print(count)
...
def reserve_5(preds, out_path='', reserve=5):
    preds[:, :4] = xyxy2xywh(preds[:, :4])
    with open(out_path, 'w') as f:
        reserve_count = {}
        count = 0
        for p in preds:
            box_id = get_box_id(p)
            if box_id not in reserve_count:
                reserve_count[box_id] = 1
            elif reserve_count[box_id] >= reserve:
                continue
            else:
                reserve_count[box_id] += 1
            coord = ''
...
def get_box_id(p):
    return str(int(p[4])) + '-' + str(int(p[-1]))


def enhance_pred(in_path):
    input_path = in_path + '/pred_yolo.txt'
    out_path = in_path + '/pred_enhance_yolo.txt'
    with open(input_path, 'r') as input_f:
        with open(out_path, 'w') as out_f:
            ...
            for l in lines:
                es = l.split(',')
                img_i = int(float(es[-2]))
                if img_i == img_id:
                    coords = ''
                    area = float(es[2]) * float(es[3])
                    for c in es[:4]:
                        coords += format_float(float(c)) + ','
                    coords += format_float(float(es[4])) + ',' + str(img_i) + ',' + \
                          str(int(float(es[-1].replace('\n', '')))) + ',' + format_float(area) + ',' + str(box_id) + '\n'
                    out_f.write(coords)
                    box_id += 1
                else:
                    img_id = img_i
...
def box_with_defects(input_path='result_txt/box_with_defects.txt',
                     out_path='result_txt/pred_box_with_defects_yolo.txt',
                     preds=None):
    preds[:, :4] = xyxy2xywh(preds[:, :4])
    with open(input_path, 'r') as i_f:
        with open(out_path, 'w') as o_f:
            if_lines = i_f.readlines()
            for l in if_lines:
                if l is None or l == '':
                    return
                l = l.replace('\n', '')
                es = l.split(',')
                img_id = int(es[0].split('-')[0])
                box_id = int(es[0].split('-')[1])
                cs = [label_names.index(i) for i in es[1:]]
                p = preds[(preds[:, 4] == img_id) & (preds[:, 7] == box_id)][0]
                for c in cs:
                    p[5] = c
                    o_f.write(predline_to_str(p))
...
def get_args():
    ...
    parser.add_argument('-d', type=str, help='BKS_DIR', required=True)
...
if __name__ == '__main__':
    args = get_args()
    input_path = args.d
    out_path = os.path.join(input_path, '4cols_post')
    # input_path = 'runs/val/Mixed_data/7X+8X+EVT_MICSUS/BKS/'
    # out_path = 'runs/val/Mixed_data/7X+8X+EVT_MICSUS/4cols_post'
    ...
    calc_indicators(
        ...
        conf_thres=0,
        merge=True,  # 关键
        # merge_file='pred_enhance_',
        # merge_file='pred_box_with_defects_', # 关键
        # merge_file='pred_filter_',
